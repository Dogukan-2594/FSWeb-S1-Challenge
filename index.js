...
function indekseGoreFenomen(fenomenArray,number) {

return `${number}. indekste bulunan fenomen: ${fenomenArray[number].profile}`;
...
function fenomenEkle(fenomenler,number,profil,followers,posts,platform) {
  const newArray=[...fenomenler];
  const newObject={
...
console.log( fenomenEkle(fenomenler, 6, "emre şahiner", 10000000, 2022, "Instagram"));
...
/*function fenomenGonderimSayisi(fenomenler,isim){

   for (let name in fenomenler) {

      if(name == isim){
        return  fenomenler[posts];
      } 
    }
...
function platformaGoreCokGonderiYapanFenomen(fenomenler,platform){
  ...
  let enBuyuk =Math.max(...newArray);
    for(let i=0;fenomenler.length;i++){
      if (enBuyuk == fenomenler[i].posts){
       return  fenomenler[i].profile;
      }
    }
  */
